...
interface UomConverterProps {
  ...
  initialValues?: InitialValues;
  isPurchase?: boolean;
  readOnly?: boolean | ReadOnlyConfig;  // ✅ NEW: readOnly prop
}
...
const UomConverter = ({
  ...
  // ✅ NEW: Helper to check if field is readOnly
  const isReadOnly = (field: 'primary' | 'secondary' | 'tertiary'): boolean => {
    if (typeof readOnly === 'boolean') return readOnly;
    return readOnly?.[field] ?? false;
  };

  // ✅ NEW: Check if ALL fields are readOnly
  const isAllReadOnly = (): boolean => {
    if (typeof readOnly === 'boolean') return readOnly;
    return (readOnly?.primary && readOnly?.secondary && readOnly?.tertiary) ?? false;
  };
  ...
  const notifyParent = (uom1: string, uom2: string, uom3: string, selectedSaleUnit: string) => {
    if (onChange) {
      ...
      const data = {
        ...
        uom2_qty: parseFloat(uom2) || 0,
        uom3_qty: parseFloat(uom3) || 0,
        sale_unit: parseInt(selectedSaleUnit),
        Uom_Id: uomId
      };
  ...
  const handleSaleUnitChange = (unit: string) => {
    if (isAllReadOnly()) return;  // ✅ NEW: Block if all readOnly
    console.log(`🎯 Line ${lineIndex} - Sale Unit changed to: ${unit}`);
    setSaleUnit(unit);
    notifyParent(uom1Val, uom2Val, uom3Val, unit);
  };
  ...
  // ✅ NEW: Get input className based on readOnly
  const getInputClass = (field: 'primary' | 'secondary' | 'tertiary', isSelected: boolean, baseWidth: string) => {
    const fieldReadOnly = isReadOnly(field);
    
    const baseClass = `border rounded-md px-1 py-0.5 h-8 text-sm focus:ring-1 transition-all ${baseWidth}`;
    
    if (fieldReadOnly) {
      return `${baseClass} bg-gray-100 text-gray-600 border-gray-300 cursor-not-allowed`;
    }
    
    if (isSelected && !isPurchase) {
      return `${baseClass} border-green-400 bg-green-50 focus:ring-green-500`;
    }
    ...
    <div className="flex items-start">
      ...
      {uomData.tertiary && (
        <div className="gap-2 border-gray-400 px-1.5">
          ...
          <input
            ...
            onChange={handleUom3Change}
            placeholder="0"
            readOnly={isReadOnly('tertiary')}  // ✅ Native readOnly
            className={getInputClass('tertiary', saleUnit === '3', 'w-14')}
          />
        </div>
      )}
      ...
      {uomData.secondary && (
        <div className="gap-2 border-gray-400 px-1.5">
          ...
          <input
            ...
            onChange={handleUom2Change}
            placeholder="0"
            readOnly={isReadOnly('secondary')}  // ✅ Native readOnly
            className={getInputClass('secondary', saleUnit === '2', 'w-16')}
          />
        </div>
      )}
      ...
      <div className="gap-2 border-gray-400 px-1.5">
        ...
        <input
          ...
          onChange={handleUom1Change}
          placeholder="0"
          readOnly={isReadOnly('primary')}  // ✅ Native readOnly
          className={getInputClass('primary', saleUnit === '1', 'w-[80px]')}
        />
      </div>
    </div>
  );
