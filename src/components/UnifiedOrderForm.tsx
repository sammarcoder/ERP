...
const UnifiedOrderForm = ({ orderType }: { orderType: string }) => {
  ...
  // ✅ FIXED: Item selection handler (fixes UomConverter itemId issue)
  // const handleItemSelection = (index: number, name: string, value: any) => {
  //   console.log('🎯 Item selected for row', index, ':', { name, value });
  ...
  //     // ✅ CRITICAL: Use OrderLogic to setup item defaults properly
  //     updatedDetails[index] = OrderLogic.setupItemDefaults(
  //       updatedDetails[index],
  //       selectedItem,
  //       selectedAccount,
  ...
  //     // Auto-expand row to show UOM converter
  //     setExpandedRows(prev => new Set([...prev, index]));
  ...
  // In your UnifiedForm.tsx, update the handleItemSelection method:
  ...
  // ✅ FIXED: Form submission using OrderAPI and OrderLogic
  // const handleSubmit = async (e: React.FormEvent) => {
  //   e.preventDefault();

  //   // Validate using OrderLogic
  //   const validation = OrderLogic.validateForm(master, details);
  //   if (!validation.isValid) {
  //     setMessage({ type: 'error', text: validation.message });
  //     return;
  ...
  //   // ✅ FIXED: Prepare order data in correct format for your API
  //   const orderData = {
  //     master: {
  //       Date: master.Date,
  //       COA_ID: Number(master.COA_ID),
  ...
  //     },
  //     details: details
  //       .filter(d => d.Item_ID && d.uom2_qty > 0) // Only valid items
  //       .map(d => ({
  //         Item_ID: Number(d.Item_ID),
  //         Price: Number(d.Price),
  //         Stock_In_UOM: d.Stock_In_UOM ? Number(d.Stock_In_UOM) : null,
  ...
  //   try {
  //     // ✅ Use OrderAPI with your actual endpoints
  ...
  //     if (result.success) {
  //       console.log('✅ Order operation successful, redirecting...');
  //       setTimeout(() => {
  //         router.push(`/order/${orderType}`);
  //       }, 2000);
  //     } else {
  //       console.error('❌ Order operation failed:', result.message);
  //     }
  //   } catch (error) {
  //     console.error('❌ Submit error:', error);
  //     setMessage({ type: 'error', text: 'Failed to submit order' });
