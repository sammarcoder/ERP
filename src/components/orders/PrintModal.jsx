...
// const PrintModal = ({ order, isPurchase, onClose }) => {
...
//   // ✅ Integrated UOM Display Logic from Existing System
...
//   useEffect(() => {
//     if (order?.Transporter_ID && order?.account) {
//       // Set transporter info from order if available
//       const transporterText = `Transporter ID: ${order.Transporter_ID} | Freight: ${order.freight_crt || 0} | Labour: ${order.labour_crt || 0}`
//       setTransporterInfo(transporterText)
//     }
//   }, [order])
...
//             </div>
//             <div class="customer-row">
//               <span><strong>Sub:</strong> ${order.sub_customer || order.account?.sub_customer || 'N/A'}</span>
//               <span><strong>City2:</strong> ${order.sub_city || order.account?.sub_city || 'N/A'}</span>
//             </div>
//             <div class="ada-box">
...
//               <strong>Total Items:</strong> ${totalItems} | <strong>Est. Value:</strong> Rs ${totalAmount.toFixed(2)}
//             </div>
//             <div>
...
//                 <strong>Paper Size:</strong> 4" x 7" (Custom) | <strong>Order:</strong> {order.Number}
//               </p>
//             </div>
//           </div>
//         </div>
...
const directPrintOrder = (order, isPurchase = false) => {
 ...
 function formatMyDate(dateString) {
  ...
  const transporterInfo = order?.Transporter_ID 
    ? `Transporter ID: ${order.Transporter_ID} | Freight: ${order.freight_crt || 0} | Labour: ${order.labour_crt || 0}`
    : ''
...
// // components/DirectPrintModal.jsx - SIMPLE & CLEAN WITH react-to-print
// import React, { useRef } from 'react';
// import { useReactToPrint } from 'react-to-print';

// const DirectPrintModal = ({ order, isPurchase = false }) => {
...
//   // ✅ UOM Display Logic (same as before)
//   const getDisplayQuantity = (detail) => {
//     const saleUnit = parseInt(detail.sale_unit);
//     const uomId = detail.Uom_Id;
//     let quantity = 0;
...
//   // ✅ Simple print handler - ONE LINE!
//   const handlePrint = useReactToPrint({
//     content: () => componentRef.current,
//     documentTitle: `${isPurchase ? 'Purchase-Order' : 'Sales-Order'}-${order.Number}`,
//     pageStyle: `
//       @page {
//         size: 4in 7in;
//         margin: 0.2in;
...
//   // ✅ Auto-print when component mounts (direct print)
//   React.useEffect(() => {
//     handlePrint();
//   }, [handlePrint]);

//   // ✅ Simple JSX - No complex HTML strings!
//   return (
...
//             <span><strong>Date:</strong> {formatMyDate(order.Date)}</span>
//             <span><strong>Order:</strong> {order.Number}</span>
//             <span style={{ color: order.Next_Status === 'Complete' ? 'green' : 'orange' }}>
//               <strong>Status:</strong> {order.Next_Status}
//             </span>
//           </div>
          
//           <hr style={{ margin: '6px 0', borderTop: '1px solid #333' }} />
          
//           <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10pt', marginBottom: '5px' }}>
//             <span><strong>{isPurchase ? 'Supplier' : 'Customer'}:</strong> {order.account?.acName || 'N/A'}</span>
//             <span><strong>City:</strong> {order.account?.city || 'N/A'}</span>
//           </div>
          
//           <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10pt', marginBottom: '5px' }}>
//             <span><strong>Sub:</strong> {order.sub_customer || 'N/A'}</span>
//             <span><strong>City2:</strong> {order.sub_city || 'N/A'}</span>
//           </div>
          
//           {/* ADA Box */}
//           <div style={{ border: '1px solid #666', padding: '4px', margin: '6px 0' }}>
//             <div style={{ fontWeight: 'bold', fontSize: '9pt', marginBottom: '2px' }}>ADA:</div>
//             <div style={{ minHeight: '16px', fontSize: '9pt', padding: '2px 0' }}>
...
//           margin: '6px 0' 
//         }}>
//           {isPurchase ? 'PURCHASE ORDER ITEMS' : 'SALES ORDER ITEMS'}
//         </div>
...
//           <thead>
//             <tr>
//               <th style={{ border: '1px solid #000', background: '#e8e8e8', padding: '3px 2px', fontSize: '9pt', width: '7%' }}>Sr</th>
//               <th style={{ border: '1px solid #000', background: '#e8e8e8', padding: '3px 2px', fontSize: '9pt', width: '42%', textAlign: 'left' }}>Item Name</th>
//               <th style={{ border: '1px solid #000', background: '#e8e8e8', padding: '3px 2px', fontSize: '9pt', width: '8%' }}>✓</th>
//               <th style={{ border: '1px solid #000', background: '#e8e8e8', padding: '3px 2px', fontSize: '9pt', width: '23%' }}>Quantity</th>
//               <th style={{ border: '1px solid #000', background: '#e8e8e8', padding: '3px 2px', fontSize: '9pt', width: '20%' }}>Dispatch</th>
//             </tr>
//           </thead>
//           <tbody>
//             {order.details?.map((detail, idx) => (
//               <tr key={detail.ID} style={{ background: idx % 2 === 0 ? 'white' : '#f9f9f9' }}>
//                 <td style={{ border: '1px solid #000', padding: '3px 2px', fontSize: '9pt', textAlign: 'center' }}>
//                   <strong>{idx + 1}</strong>
//                 </td>
//                 <td style={{ border: '1px solid #000', padding: '3px 2px', fontSize: '9pt', textAlign: 'left' }}>
//                   {detail.item?.itemName || 'Unknown'}
//                 </td>
//                 <td style={{ border: '1px solid #000', padding: '3px 2px', fontSize: '9pt', textAlign: 'center' }}>
//                   <span style={{ width: '11px', height: '11px', border: '1px solid #000', display: 'inline-block' }}></span>
//                 </td>
//                 <td style={{ border: '1px solid #000', padding: '3px 2px', fontSize: '9pt', textAlign: 'center' }}>
//                   <strong>{getDisplayQuantity(detail)}</strong>
//                 </td>
//                 <td style={{ border: '1px solid #000', padding: '3px 2px', fontSize: '8pt' }}>
//                   {/* Empty dispatch column */}
//                 </td>
//               </tr>
//             ))}
            
//             {/* Fill empty rows */}
//             {Array.from({ length: Math.max(0, 16 - (order.details?.length || 0)) }, (_, i) => (
//               <tr key={`empty-${i}`} style={{ background: (order.details?.length + i) % 2 === 0 ? 'white' : '#f9f9f9' }}>
//                 <td style={{ border: '1px solid #000', padding: '3px 2px', fontSize: '9pt', textAlign: 'center' }}>&nbsp;</td>
//                 <td style={{ border: '1px solid #000', padding: '3px 2px', fontSize: '9pt' }}>&nbsp;</td>
//                 <td style={{ border: '1px solid #000', padding: '3px 2px', fontSize: '9pt', textAlign: 'center' }}>
//                   <span style={{ width: '11px', height: '11px', border: '1px solid #000', display: 'inline-block' }}></span>
//                 </td>
//                 <td style={{ border: '1px solid #000', padding: '3px 2px', fontSize: '9pt' }}>&nbsp;</td>
...
//             <strong>Total Items:</strong> {order.details?.length || 0} | <strong>{isPurchase ? 'PO' : 'SO'}:</strong> {order.Number}
//           </div>
//           <div>Print: {new Date().toLocaleString()}</div>
//         </div>
//       </div>
//     </div>
//   );
